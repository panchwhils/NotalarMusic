...
class YouTube:
    ...
    def get_cookies(self):
        if not self.checked and os.path.isdir(self.cookie_dir):
        ...
        async with aiohttp.ClientSession() as session:
            for i, url in enumerate(urls):
                try:
                    ...
                    link = f"https://batbin.me/api/v2/paste/{paste_id}"
                    path = f"{self.cookie_dir}/cookie_{i}.txt"

                    async with session.get(link, timeout=15) as resp:
                        if resp.status != 200:
                            logger.warning("Failed to fetch cookie %s (%s)", url, resp.status)
                            continue
                        with open(path, "wb") as fw:
                            fw.write(await resp.read())
                except Exception as e:
                    logger.warning("Cookie fetch error: %s", e)
    ...
    async def search(
        ...
        return Track(
            ...
            thumbnail=(data.get("thumbnails") or [{}])[-1]
            .get("url", "")
            .split("?")[0],
            url=data.get("link"),
